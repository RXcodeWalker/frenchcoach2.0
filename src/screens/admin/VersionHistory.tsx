...
export function VersionHistory({ kind }: { kind: 'questions' | 'scenarios' }) {
  ...
  return (
    <div className="max-w-3xl pb-16">
      ...
      <p className="text-sm text-ink-muted mb-6 font-mono">{id}</p>
      ...
      {loading ? (
        ...
        <p className="text-sm text-ink-muted py-6">No prior versions — this record hasn't been edited yet.</p>
      ) : (
          ...
          {versions.map(v => (
            <li key={v.version_number} className="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-900/60 px-4 py-3">
              <div>
                ...
                <span className="text-xs text-ink-muted ml-3">{new Date(v.created_at).toLocaleString()}</span>
              </div>
      ...
      {preview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-2xl max-h-[80vh] overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900 p-6">
            ...
            <p className="text-sm text-ink-muted mb-4">{preview.diff.length} field(s) will change.</p>
            <div className="rounded-lg border border-slate-800 overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-slate-800 text-xs uppercase text-ink-muted">
                ...
                <tbody>
                  {preview.diff.map(d => (
                    <tr key={d.field} className="border-t border-slate-800 bg-amber-500/5">
                      ...
                      <td className="p-2 text-ink-muted max-w-[200px] truncate">{fmt(d.from)}</td>
                      <td className="p-2 text-amber-300 max-w-[200px] truncate">{fmt(d.to)}</td>
                    </tr>
                  ))}
                  {preview.diff.length === 0 && <tr><td colSpan={3} className="p-4 text-center text-ink-muted">No differences.</td></tr>}
                </tbody>
              </table>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setPreview(null)} className="px-4 py-2 rounded-lg text-sm font-semibold text-ink-muted hover:bg-slate-800">Cancel</button>

      ...
      >
        ...
        <motion.div variants={fadeUp} className="rounded-xl glass p-4">
          ...
          <div className="space-y-0.5">
            ...
            <SettingToggle icon={<Moon size={14} />} label="Dark Mode" description="Always on" enabled={true} onToggle={() => {}} disabled />
...
function SettingToggle({ icon, label, description, enabled, onToggle, disabled }: { icon: React.ReactNode; label: string; description: string; enabled: boolean; onToggle: () => void; disabled?: boolean }) {
  return (
    <div className={`flex items-center gap-3 p-2.5 rounded-lg transition-colors ${disabled ? 'opacity-35' : 'hover:bg-white/[0.02] cursor-pointer'}`} onClick={disabled ? undefined : onToggle}>
      <div className="text-slate-600">{icon}</div>
      <div className="flex-1"><p className="text-[10px] font-semibold text-white">{label}</p><p className="text-[9px] text-slate-700">{description}</p></div>

    ...
    <PageShell>
      ...
      >
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Learn together, reach goals faster</p>
        </div>
        ...
        <div className="flex bg-white/5 p-1 rounded-xl border border-white/5 self-start">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-bold transition-all ${
              activeTab === 'my-groups' ? 'bg-emerald-500/10 text-emerald-400' : 'text-slate-500 hover:text-slate-300'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-bold transition-all ${
              activeTab === 'discover' ? 'bg-emerald-500/10 text-emerald-400' : 'text-slate-500 hover:text-slate-300'
            }`}
          >
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-elevated p-4 rounded-2xl border-emerald-500/20">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] font-bold text-slate-500 uppercase">Group Rank</span>
          </div>
          <p className="text-2xl font-black text-white">#12 <span className="text-xs font-normal text-slate-500 italic">global leaderboard</span></p>
        </div>
        <div className="glass-elevated p-4 rounded-2xl border-blue-500/20">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] font-bold text-slate-500 uppercase">Weekly XP</span>
          </div>
          <p className="text-2xl font-black text-white">4,400 <span className="text-xs font-normal text-slate-500 italic">combined effort</span></p>
        </div>
        <div className="glass-elevated p-4 rounded-2xl border-amber-500/20">
          ...
          <p className="text-2xl font-black text-white">24 <span className="text-xs font-normal text-slate-500 italic">members online</span></p>
        </div>
      </motion.div>
        ...
        {activeTab === 'my-groups' ? (
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Ongoing Progress</h2>
            ...
            <button
              onClick={() => setActiveTab('discover')}
              className="w-full py-8 border-2 border-dashed border-white/5 rounded-2xl flex flex-col items-center justify-center gap-2 text-slate-600 hover:border-emerald-500/20 hover:text-emerald-400 transition-all group"
            >
              <Search size={24} className="group-hover:scale-110 transition-transform" />
              <div className="text-center">
                <span className="text-xs font-bold uppercase tracking-widest block">Find more groups</span>
                <span className="text-[10px] text-slate-600">Explore communities that match your goals</span>
              </div>
            </button>
          </motion.div>
        ) : (
          ...
          >
            ...
            <div className="flex flex-col md:flex-row gap-3">
              ...
              <button className="px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl text-slate-400 hover:text-white transition-all flex items-center gap-2">
                <Filter size={18} />
            ...
            {filteredDiscoverGroups.length === 0 && (
              <div className="py-20 text-center">
                ...
                <p className="text-sm text-slate-600 mt-1">Try a different search term or explore tags</p>
              </div>
            )}
          </motion.div>
        )}
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left space-y-3 min-w-[180px]">
          ...
          <div>
            <h3 className="font-bold text-white group-hover:text-emerald-400 transition-colors flex items-center gap-2">
              {group.name}
              {group.isPrivate && <Lock size={12} className="text-slate-600" />}
            </h3>
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {group.tags.map(tag => (
              <span key={tag} className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-white/5 border border-white/5 text-slate-500 group-hover:border-emerald-500/20 group-hover:text-emerald-400/70 transition-all">
                #{tag}
              </span>
            ))}
          </div>

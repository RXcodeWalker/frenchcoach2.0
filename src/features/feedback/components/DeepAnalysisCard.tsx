            ...
            <>
              ...
              {deep.sentences.length > 0 && (
                <motion.div variants={fadeUp} className="rounded-xl glass p-4">
                  <p className="text-[9px] font-bold text-slate-600 uppercase tracking-wider mb-3">Sentence Analysis</p>
                  <div className="space-y-2">
                    {deep.sentences.map((s, i) => (
                      <div key={i} className="p-2.5 rounded-lg glass-subtle">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-[9px] font-bold uppercase ${COMPLEXITY_COLOR[s.complexity] ?? 'text-slate-500'}`}>
                            {s.complexity}
                          </span>
                        </div>
                        <p className="text-[10px] text-slate-300 italic mb-1">"{s.text}"</p>
                        <p className="text-[10px] text-slate-500">{s.critique}</p>
                      </div>
                    ))}
                  </div>
                </motion.div>
              ...
              {deep.modelAnswer && (
                <motion.div variants={fadeUp} className="rounded-xl glass p-4">
                  <p className="text-[9px] font-bold text-slate-600 uppercase tracking-wider mb-2.5">Model Answer</p>
                  <p className="text-[11px] text-emerald-300 leading-relaxed italic mb-2">
                    "{deep.modelAnswer.text}"
                  </p>
                  <p className="text-[10px] text-slate-500">{deep.modelAnswer.whyItScores}</p>
                </motion.div>
              )}
              ...
              {deep.pushToTopMarks.length > 0 && (
                <motion.div variants={fadeUp} className="rounded-xl glass p-4">
                  <p className="text-[9px] font-bold text-slate-600 uppercase tracking-wider mb-2.5">
              ...
              {deep.rhythm && (
                <motion.div variants={fadeUp} className="rounded-xl glass p-4">
                  <p className="text-[9px] font-bold text-slate-600 uppercase tracking-wider mb-2">Rhythm & Pacing</p>
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-[10px] text-slate-400">{deep.rhythm.comment}</p>
                  </div>
                </motion.div>
              )}
            </>

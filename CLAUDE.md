...
- **Under-13 accounts require guardian confirmation before any audio capture / provider call.** The gate is `profiles.consent_status` (never client-writable) + `SpeakingConsentGate` wrapping the record control, backed by a `useRecording(blocked)` guard. Browser mic permission is never treated as consent — see `docs/systems/child-safety-consent.md` and ADR 0006. `SpeakingConsentGate`/the `blocked` guard are wired into Learn, ExamMode, RoleplaySession, StoryMode, ScenarioArchitectSession, and DailyNewsFlash — **not** into `useAudioBlobRecorder` (`SayItAgainCard.tsx`'s practice-step recorder) or SpeakingArena/SpeedSpeaking. That's a known gap, not a design decision.

...
|---|---|---|---|---|---|---|
| **Streak Freeze** | `analyticsService.updateStreak` | Automatically, on the first answer of a day that follows a 1-day gap | `user_inventory.qty − 1` (server) + local streak count +1 | Inventory yes (server); streak is **localStorage-only** and diverges per device | Prevented server-side by `consume_item`'s replay guard + `qty > 0` check in one transaction — this is what makes consumption exactly-once, not client awaiting. **A9 fixed (amended during Phase 4):** `hasStreakFreeze()` reads a server-reconciled local cache (`shopInventoryCache`, written by `AppContext.refetchEconomy` after every `SET_ECONOMY`) instead of the stale merged `inventory` JSONB; `consumeItem()` fires `consume_item` without awaiting it and optimistically decrements that same cache. `updateStreak`/`recordSession`/`orchestrateAttempt` stay synchronous — deliberately not threaded through `await`, because `orchestrateAttempt` is a documented synchronous, no-dispatch contract relied on by 4 screens and their tests. **Accepted window:** if the cache is stale (e.g. a freeze was already spent on another device before this device's next hydrate), the streak can be preserved locally for up to one day before reconciling back down. This is a display correction only — no gems or inventory are actually granted twice, because `consume_item`'s server-side guard prevents a second real decrement regardless of what the client believes. | No |
...
Layout, header, 3 tabs, `ShopItemCard` rewrite, purchase flow with real error states. Rewire Streak Freeze to server inventory (fixes A9) — via a synchronous read of a server-reconciled local cache plus a fire-and-forget `consume_item` call, **not** by making `updateStreak`/`recordSession`/`orchestrateAttempt` async (see §14.4 for why, and the accepted reconciliation window this implies).
...
**Precondition (added during Phase 2 implementation, resolved in Phase 4):** between Phase 2 and Phase 4, `consumeStreakFreeze` was local-only (see the §15 Phase 4 deferral note) and could not demonstrate the server-backed, non-restorable-by-cloud-pull property test 5 below describes. As of Phase 4, `consumeItem` calls `consume_item` (fire-and-forget, per §14.4's amended row) so test 5 is unblocked — but it verifies server state directly (`user_inventory.qty`), not just the UI-visible streak count, since the client never awaits the RPC's confirmation.
...
5. Buy a Streak Freeze, skip a day, return → streak intact, `user_inventory.qty` for `streak_freeze` decreased by exactly 1 (check via SQL/dashboard, not just the UI), the freeze **not** restored by the next cloud pull (A9), and a second app load on the same day does **not** decrement `qty` again. *(Amended during Phase 4 implementation — the original wording only asserted "not restored by a cloud pull"; the real invariant enforced by `consume_item`'s server-side guard is exactly-once decrement, which this step now checks directly rather than only its cloud-pull symptom.)*

...
Dependency notes: S3 before S6 (recordings need ingest). S10 and S11 are deliberately after Phase B — teacher-conducted recordings, not app sessions, feed Phases A/B, so the scoring pipeline outranks the practice-session engine until calibration exists.

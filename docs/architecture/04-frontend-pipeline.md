...
- **Build an original question bank modelled on the published structure**, not copied from it. Role play: a French scene-setting `setup` (who the candidate is, the situation, and who the examiner plays — read aloud and shown on the prep card, mirroring the real 0520/03 candidate card, which is scenario-only with no task list) plus 5 transactional questions the examiner asks the candidate, drawn from the syllabus topic areas (A: Everyday activities; B: Personal and social life; C: World around us; D: World of work; E: International world), following the same functional progression (open/greet, ask a detail, offer a choice between two options, ask a follow-up, close). Topic conversations: 5 open questions per topic with one alternative per main question (Cambridge requires alternatives to support weaker candidates).
...
- The exam has a fixed non-assessed opening (`GREETING_TEXT` in `ExamMode.tsx`), a non-assessed role-play scene-setting beat (`rolePlay.setup`, spoken the same way — UI-layer, before `SimulationSession.begin()`, never logged to `ConductLog`), and a fixed closing (`EXAM_CLOSING_TEXT` in `conductEngine.ts`), so every session begins and ends with a clear, professional signal.

...
- **Time-frame alignment (per question):** each question in the question bank carries an `expected_time_frame` tag (`past` | `present` | `future` | `conditional`) derived from its cue words (e.g. *récemment*, *la semaine dernière*, *d'habitude*, *à l'avenir*, *aimerais-tu*). The detector classifies the dominant verb time frame of the candidate's response for that question and emits `aligned` | `misaligned` | `no_verb`. **Rationale (Principal Examiner Report):** a verb in the wrong time frame after cue words renders the response ambiguous → 1 mark instead of 2 on the affected role-play task, or impairs Communication/QoL band placement on topic questions. **`no_verb` is not a penalty** — brief verbless answers can score full marks; the detector reports absence of a classifiable verb, it does not infer failure.
...
- `role_play_task_mark ≤ 1` for any task where L1 reports `time_frame_alignment: misaligned` (Principal Examiner Report: verb in wrong time frame after cue words → partial communication, not full).
